...
details, including 
command-line flags, how to use the shared library interface, and the agent/environment protocol 
...
\begin{itemize}
  ...
  \item{\textbf{C++ agent / RL-Glue interface.} See Section \ref{sec:rlglue_interface} and \verb+doc/examples/RLGlueAgent.c+.}
  \item{\textbf{C++ agent / shared library interface.} See Section \ref{sec:shared_library_interface} and \verb+doc/examples/sharedLibraryInterfaceExample.cpp+.}
\end{itemize}

\section{Command-line Arguments}

Command-line arguments can be passed to ALE before the ROM name. For example, the following runs
...
The following command-line arguments to ALE are available.
...
\begin{verbatim}
  ...
  -system_reset_steps ### -- how many frames to hold reset button for. Should be >= 2.
    default: 2

  -use_environment_distribution <true|false> -- if true, the environment start 
      state is drawn from a small distribution of states.
    default: false
  ...
  -backward_compatible_save <true|false> -- uses ALE 0.2's state saving/loading 
      mechanism.
    default: false

  -disable_color_averaging <true|false> -- if true, color averaging is disabled
    default: false
\end{verbatim}
}
...
These agents are the random agent, the single-action agent and the keyboard agent\footnote{Requires SDL support.}.
...
where the first integer is player A's action -- \textsc{fire} -- and the second integer, player B's action -- \textsc{noop}. Emulator control (reset, save/load state) is also handled by sending a special action value as player A's action. See Section \ref{sec:available_actions} for the list of available actions.
...
RL-Glue mode. It will then communicate with the RL-Glue core like a normal RL-Glue environment.

\subsection{Sample Agent and Experiment}

Example source code (copied near-verbatim from the skeleton agent and experiment provided
with the C/C++ RL-Glue codec) can be found under
...
The action space consists of both Player A and Player B's actions (see \ref{sec:available_actions}
for details). In general, Player B's action may safely be set to noop (18). The observation
space consist in 33728 integers: first the 128 bytes of RAM (taking values in 0--255), then the 
...
colour averaging is enabled: the environment output (as observed by agents) averages the last
two frames. This behaviour can be turned off using the command-line argument 
