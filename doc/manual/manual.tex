...
clear.
  ...
  \textbf{If the screen size is different from the vector size an error will occur}.
  
  \verb+void getScreenRGB(std::vector<std::vector<unsigned char> >& output_rgb_buffer)+:\\ 
  This method receives a vector of size 3 and each one of the three positions must have a
  vector of size width $\times$ height (generally $160 \times 210 = 100,800$). The first positions
  contain the red colours, followed by the green colours and then the blue colours. \textbf{If the
  screen size is different from the vector size an error will occur}.
