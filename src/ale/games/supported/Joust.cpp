...
void JoustSettings::step(const System& system) {
  ...
  // the lives bytes wrap to 0xff when the last life is lost
  lives_p1 = static_cast<int>(static_cast<signed char>(readRam(&system, 0x81)));
  lives_p2 = static_cast<int>(static_cast<signed char>(readRam(&system, 0x82)));
...
// The even game variants are the single-player games; the odd variants are
// their two-player equivalents (RAM 0xF8 holds the variant, modulo 4).
