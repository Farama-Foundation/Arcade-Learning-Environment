...
void ColourPalette::applyPaletteRGB(std::vector<std::vector<unsigned char> >& dst_buffer, uInt8 *src_buffer, size_t src_size)
{
    uInt8 *p = src_buffer;

    for(size_t i = 0; i < src_size; i++, p++){
        int rgb = m_palette[*p];
        dst_buffer[0][i] = (unsigned char) ((rgb >> 16));    // r
        dst_buffer[1][i] = (unsigned char) ((rgb >>  8));    // g
        dst_buffer[2][i] = (unsigned char) ((rgb >>  0));    // b
    }
}
